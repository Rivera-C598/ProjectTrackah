window.downloadAspDocx = async function (group, section, members) {
  ...
  const memberList   = members || (group && group.members) || [];
  ...
  const membersTable = makeTable(
    ...
    memberList.length > 0
      ? memberList.map(m => [m.fullName || m, "[e.g. Backend Developer / UI Designer]", "[Describe what this member built or handled]"])
  ...
  const tablesTable = makeTable(
    ...
    [
      ["[e.g. Products]", "[Main table — stores product records]", "[Id, Name, Price, CategoryId, CreatedAt]"],
      ["[e.g. Categories]", "[Lookup table — referenced by main table via FK]", "[Id, CategoryName]"],
      ["[Add more tables]", "", ""]
    ]
  );
  ...
  const linksTable = makeTable(
    ...
    [
      ...
      ["SQL / Migration File", "[e.g. FinalProject_db.sql  or  Migrations folder in project]"],
  ...
  const doc = new Document({
    sections: [{
      ...
      children: [
        ...
        ph("[List 3–5 specific goals your system achieves. e.g. Allow administrators to manage records, Provide a clean interface for data entry, Ensure data integrity through validation]"),
        ...blankLines(4),
        ...
        bodyText("Paste or draw your Entity-Relationship (ER) diagram below (optional but recommended):"),
        ...blankLines(6),

        // 6. Screenshots
        sectionHeading("6. Screenshots"),
        bodyText("Paste screenshots of each main page below. Label each one clearly."),
        subHeading("6.1 Home / Index Page"),
        ...blankLines(7),
        subHeading("6.2 List / Index View"),
        ...blankLines(7),
        subHeading("6.3 Create Form"),
        ...blankLines(7),
        subHeading("6.4 Edit Form"),
        ...blankLines(7),
        subHeading("6.5 Delete Confirmation"),
        ...blankLines(5),
        subHeading("6.6 Validation Error (at least one)"),
        ...blankLines(5),
        subHeading("6.7 Other pages (if any)"),
        ...blankLines(5),

        // 7. How to Run Locally
        sectionHeading("7. How to Run Locally"),
        bodyText("Follow these steps to run the project on a local machine using Visual Studio and SQL Server:"),
        new Paragraph({ children: [run("1.", { bold: true }), run("  Clone or download the project from GitHub.")], spacing: { after: 60 } }),
        new Paragraph({ children: [run("2.", { bold: true }), run("  Open the solution file ("), run(".sln", { italic: true }), run(") in Visual Studio 2022 or later.")], spacing: { after: 60 } }),
        new Paragraph({ children: [run("3.", { bold: true }), run("  Open "), run("appsettings.json", { italic: true }), run(" and update the connection string with your SQL Server instance name.")], spacing: { after: 60 } }),
        new Paragraph({ children: [run("4.", { bold: true }), run("  Open the Package Manager Console and run: "), run("Update-Database", { bold: true, color: ACCENT }), run(" to apply migrations.")], spacing: { after: 60 } }),
        new Paragraph({ children: [run("5.", { bold: true }), run("  Press "), run("F5", { bold: true }), run(" or click Run to launch the application in the browser.")], spacing: { after: 80 } }),
  ...
  a.href = url;
  a.download = `${groupName.replace(/\s+/g, "_")}_ASP_Documentation.docx`;
  a.click();
